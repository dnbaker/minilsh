...
SLEEFLIB = sleefdir + "/build/lib/libsleef.a"
...
if not path.isfile(SLEEFLIB):
    ...
    if not path.isdir(sleefdir):
        ...
        print(sleefdir + ":" + check_output(["ls", sleefdir]).decode())
    raise RuntimeError("Expected dir to exist. Sleef lib isn't there." + SLEEFLIB);
...
include_dirs=[
   ...
   f"{minicoredir}/",
   f"{minicoredir}/python/",
   f"{minicoredir}/include",
   f"{minicoredir}/include/minicore",
   f"{minicoredir}/blaze",
   f"{minicoredir}/pybind11/include"
]
